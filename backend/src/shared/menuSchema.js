...
const BG_PATTERNS = ['none', 'torn-paper', 'torn-edge']
...
function fontOptionsWith(libraryFonts) {
...
const ZONE_DECORATION_EDGES = ['top', 'right', 'bottom', 'left', 'full']
const ZONE_DECORATION_EDGE_LABELS = {
  top: 'Bord haut',
  right: 'Bord droit',
  bottom: 'Bord bas',
  left: 'Bord gauche',
...
const ZONE_DECORATION_REPEATS = ['repeat', 'stretch']
const ZONE_DECORATION_REPEAT_LABELS = {
  repeat: 'Répéter le motif',
  stretch: 'Étirer une fois',
}

const ZONE_DECORATIONS_MAX = 8
const DECORATION_SIZE_MIN = 2
const DECORATION_SIZE_MAX = 400
const DECORATION_SIZE_FALLBACK = 40
...
function zoneStyleValue(backgroundStyle, key) {
...
const SEAM_THICKNESS_DEFAULT = 26
const SEAM_THICKNESS_MIN = 8
const SEAM_THICKNESS_MAX = 140
const SEAM_BEND_MAX = 35
...
function elementDividerStyle(el) {
  const e = el || {}
...
function seamThicknessValue(value) {
...
function seamHandles(seam, shape, thickness) {
...
function seamShapeDrag(shape, t, delta) {
...
function seamTearPath(seam, shape, thickness) {
  const axes = seamAxes(seam, shape, thickness)
...
function seamRibbon(seam, shape, thickness) {
  const axes = seamAxes(seam, shape, thickness)
  const th = axes.th
  const half = th / 2
  const amp = th * 0.55
...
function zoneSeamEdges(zones, hiddenSeams, seams) {
  const hidden = {}
...
function zoneSeamClip(zone, seams, seamShape, thickness, hiddenSeams, mode) {
...
function validateZoneDecorations(decorations, path) {
